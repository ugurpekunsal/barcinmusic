...
if (!document.getElementById("EmbedSocialIFrame")) {
	...
	(jsEmbed.id = "EmbedSocialIFrame"),
		(jsEmbed.src = "js/embed-social/iframe.js"),
...
if (!document.getElementById("EmbedSocialIFrameLightboxCSS")) {
	...
	(cssEmbed.id = "EmbedSocialIFrameLightboxCSS"),
		...
		(cssEmbed.href = "js/embed-social/iframe-lightbox.min.css");
...
EMBEDSOCIALHASHTAG = {
	getEmbedData: function (albumRef, albumDiv) {
		...
		if (iframes.length <= 0) {
			...
			var iframeId =
				"embedIFrame_" + albumRef + Math.random().toString(36).substring(7);
			var srcIfrm =
				"https://embedsocial.com/api/pro_hashtag/" +
				albumRef +
				"/" +
				queryParamString;
			...
			ifrm.setAttribute("title", "EmbedFeed widget");
			...
			for (var j = 0; j < iframes.length; j++) {
				...
				EMBEDSOCIALHASHTAG.initResizeFrame(
					albumRef,
					iframes[j].getAttribute("id")
				);
				EMBEDSOCIALHASHTAG.initFreeLinkClass(albumDiv);
			}
		}
			...
			iFrameResize(
				{
					messageCallback: function (messageData) {
						...
						if (messageData.message.hasOwnProperty("navigationCode")) {
							EMBEDSOCIALHASHTAG.navigationLightBox(
								albumRef,
								messageData.message.navigationCode
							);
						}
					...
					onLoaded: function (iframe) {
						...
						function lightBoxIframeResize() {
							...
							var iframeEl = document.getElementById(
								"embed-lightbox-iframe-" + data.albumRef
							);
	...
	closeLightBox: function (data) {
		...
		if (data.activePage && data.activePage != -1) {
			document.getElementById(data.iframeId).src =
				"https://embedsocial.com/api/pro_hashtag/" +
				data.albumRef +
				"/" +
				data.activePage;
	...
	addFreeBanner: function (data) {
		if (false) {
			...
			if (iframe) {
				var freeBanners =
					iframe.parentElement.getElementsByClassName("feed-powered-by-es");
				if (freeBanners.length === 0) {
					var freeLinkElement = document.createElement("a");
					freeLinkElement.setAttribute(
						"class",
						data.freeLinkClasses ? data.freeLinkClasses : "feed-powered-by-es"
					);
					...
					freeLinkElement.setAttribute(
						...
						data.freeLinkHref
							? data.freeLinkHref
							: "https://embedsocial.com/social-media-aggregator/"
					);
					freeLinkElement.setAttribute(
						"title",
						data.freeLinkTitle ? data.freeLinkTitle : "Instagram widget"
					);
					freeLinkElement.innerHTML =
						(data.freeLinkText ? data.freeLinkText : "Widget by EmbedSocial") +
						"<span> &#8594;</span>";
	...
	freeLinkClass: function (albumDiv) {
		...
		for (var j = 0; j < freeLinks.length; j++) {
			...
			if (
				albumDiv.offsetWidth < 768 &&
				classes.indexOf("feed-powered-by-es-center") === -1
			) {
...
if ("IntersectionObserver" in window) {
	function callVisible(e, t) {
		for (j in e)
			e[j].isIntersecting &&
				EMBEDSOCIALHASHTAG.getEmbedData(
					e[j].target.getAttribute("data-ref").trim(),
					e[j].target
				);
	}
}
function standardLoad(e) {
	for (var j = 0; j < e.length; j++) {
		...
		if (
			"yes" === t.getAttribute("data-lazyload") &&
